...
function OnPlayerDeath()
  ...
  if not LocalPlayer.state.inBattle then
    StartDeathTimer()
  end
...
function StartDeathTimer()
  ...
  if not LocalPlayer.state.IsInNoLootZone then
    earlySpawnTimer = ESX.Math.Round(30000 / 1000) 
  else
    earlySpawnTimer = ESX.Math.Round(10000 / 1000) 
  end
  ...
  CreateThread(function()
    ...
    if bleedoutTimer < 1 and isDead then
      ChooseRespawnPoint(false)
    end
  end)
end

function ChooseRespawnPoint(canChoose)
  if canChoose then
    local elements = {}

    if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
      elements[#elements+1] = {label = '! Komenda Mission Row !', coords = vector3(426.5688, -979.4356, 30.7099), heading = 89.7693}
    end

    for i=1, #Config.RespawnPoints, 1 do
      elements[#elements+1] = {label = Config.RespawnPoints[i].name, coords = Config.RespawnPoints[i].coords, heading = Config.RespawnPoints[i].heading}
    end
    ...
    ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'ambulancejob_respawn_list',
    {
      ...
      if data.current.coords then
        ...
        CreateThread(function()
          ...
          DoScreenFadeOut(800)
          RespawnPed(PlayerPedId(), data.current.coords, data.current.heading)
      ...
      ESX.ShowNotification('Wybierz miejsce odrodzenia', 'error')
    end, function()
    end, function()
...
AddEventHandler('esx_ambulancejob:revive', function(customCoords)
  RevivePlayer(customCoords)
  DoScreenFadeIn(800)
end)

function RevivePlayer(customCoords)
  local playerPed = PlayerPedId()
  local coords = customCoords or GetEntityCoords(playerPed)
  TriggerServerEvent('esx_ambulancejob:setDeathStatus', false)
...
AddEventHandler('esx:removeInventoryItem', function(item, count)
  if isDead and earlySpawnTimer > 0 then
    local PlayerInventory = ESX.GetPlayerData('inventory')
    local itemsCount = 0

    for k, v in pairs(PlayerInventory) do
      if v.count > 0 and exports['esx_policejob']:isItemInvisible(v.name) then
        itemsCount = itemsCount + 1
      else
      end
    end
    if itemsCount == 1 then
      if earlySpawnTimer > 10 then
        SendNUIMessage({
          name = "time",
          time = 10
        })
