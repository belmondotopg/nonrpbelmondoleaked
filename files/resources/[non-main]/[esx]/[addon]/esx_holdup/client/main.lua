...
AddEventHandler('esx_holdup:killBlip', function(storeBlip)
	if DoesBlipExist(blipsRobbery[storeBlip]) then
		RemoveBlip(blipsRobbery[storeBlip])
	end
end)

RegisterNetEvent('esx_holdup:setBlip')
AddEventHandler('esx_holdup:setBlip', function(position, storeBlip)
	blipsRobbery[storeBlip] = AddBlipForCoord(position)
	SetBlipSprite(blipsRobbery[storeBlip], 161)
	SetBlipScale(blipsRobbery[storeBlip], 2.0)
	SetBlipColour(blipsRobbery[storeBlip], 3)
	PulseBlip(blipsRobbery[storeBlip])
end)
					...
					if distance < 2.0 then
						...
						if IsControlJustReleased(0, 38) then
							if IsPedArmed(PlayerPedId(), 4) then
								...
								TriggerServerEvent('esx_holdup:robberyStarted', k, copsCount)
							else
