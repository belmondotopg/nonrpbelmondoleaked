...
RegisterNetEvent("EasyAdmin:RequestAdmin")
...
RegisterNetEvent('EasyAdmin:restartkurwa')

AddEventHandler('EasyAdmin:RequestAdmin', function(name)
	if name ~= nil then
		ESX.Scaleform.ShowFreemodeMessage('Zapraszam na poczekalnie', 'Zostałeś/aś zawołany/a przez: ~b~'..name, 5)
	end
end)
...
AddEventHandler('EasyAdmin:restartkurwa', function()
	if not hasSpawned then
		TriggerServerEvent("EasyAdmin:amiadmin")
	end
