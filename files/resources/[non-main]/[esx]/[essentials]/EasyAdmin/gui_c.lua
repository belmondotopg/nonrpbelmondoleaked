...
function genTxt() 
	...
	CreateRuntimeTextureFromImage(txd, 'logo', 'dependencies/img/NonEasyAdmin-2.png')
	CreateRuntimeTextureFromImage(txd, 'banner', 'dependencies/img/NonEasyAdmin-1.png')
end
	...
	if drawCustom then
		...
		CreateThread(function()
			...
			if drawCustom.vehicle and drawCustom.vehicle ~= 0 then
				...
				ShowNotification({type = 'info', text = 'Przestałeś/aś spectować'})
				cb()
			else
				Wait(1000)
				ShowNotification({type = 'info', text = 'Przestałeś/aś spectować'})
...
RegisterNetEvent("non-easyadmin:rzadowyCalled", function(admin)
...
function Initialize(scaleform)
	...
	PushScaleformMovieFunctionParameterString("Administrator Cię woła")
...
function GenerateMenu(fresh)
	...
	mainMenu = NativeUI.CreateMenu("", "~b~NON~w~RP", menuOrientation, 0, "easyadmin", "banner", "logo")
	...
	if permissions.kick or permissions.crash or permissions.ban or permissions.spectate or permissions.teleport or permissions.slap or permissions.freeze then
		...
		userSearch.Activated = function(ParentMenu, SelectedItem)
			...
			if result and result ~= "" then
				...
				ShowNotification("Nie znaleziono takiego gracza!")
			end
		end
			...
			for _, data in ipairs(players) do
				...
				if permissions.kick then
					...
					thisItem.Activated = function(ParentMenu,SelectedItem)
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'kick_reason', {
							title = "Wprowadź powód wyrzucenia (maks. 60 znaków)"
						}, function(data, menu)
							if data.value and string.len(data.value) > 0 then
								KickReason = string.sub(data.value, 1, 60)
							else
								KickReason = 'Nie określono przyczyny'
							end
					...
					local thisItem = NativeUI.CreateItem('Potwierdź wyrzucenie', '~h~UWAGA:~h~~w~ Naciśnięcie potwierdź spowoduje kopnięcie tego Gracza z określonymi ustawieniami')
					thisKickMenu:AddItem(thisItem)
					thisItem.Activated = function(ParentMenu,SelectedItem)
						if KickReason == "" then
							KickReason = 'Nie określono przyczyny'
				...
				if permissions.spectate then
                   ...
                   if permissions.spectate then
						...
						local thisItem = NativeUI.CreateItem('Skopiuj DISCORD ID do schowka',"")
						thisPlayer:AddItem(thisItem)
						thisItem.Activated = function(ParentMenu,SelectedItem)
							local discord = data.discord
							SendNUIMessage({ type = "clipboard", data = discord })
						end

						local thisItem = NativeUI.CreateItem('Skopiuj LICENCJE do schowka',"")
						thisPlayer:AddItem(thisItem)
						thisItem.Activated = function(ParentMenu,SelectedItem)
							local discord = data.rockstar
							SendNUIMessage({ type = "clipboard", data = discord })
						end

					local thisItem = NativeUI.CreateItem('Wezwij Cheatera', "")
					thisPlayer:AddItem(thisItem)
					thisItem.Activated = function(ParentMenu,SelectedItem)
						TriggerServerEvent("non-easyadmin:callRzadowy", data.id)
					end
				end
	...
	if permissions.weather then
		local thisItem = NativeUI.CreateCheckboxItem('Pogoda', "")
		settingsMenu:AddItem(thisItem)
		thisItem.CheckboxEvent = function(sender, item, status)
			if item == thisItem then
				TriggerServerEvent('niebochangee')
			end
		end
	end

	local thisItem = NativeUI.CreateItem('Skopiuj kordy x,y,z', "")
	settingsMenu:AddItem(thisItem)
	...
	local thisItem = NativeUI.CreateItem('Skopiuj kordy vector3', "")
	settingsMenu:AddItem(thisItem)
	thisItem.Activated = function(sender, item, status)
		if item == thisItem then
			local crds = GetEntityCoords(PlayerPedId())
			local str = "vector3(" .. crds[1] .. ", " .. crds[2] .. ", " .. crds[3] ..")"
			SendNUIMessage({ type = "clipboard", data = str })
			TriggerEvent("chatMessage", "🔔 EASYADMIN:", {255, 0, 0}, " Wykonano!")
		end
	end
	...
	if permissions.invisible then
		...
		-- 	local thisItem = NativeUI.CreateCheckboxItem('Ukryj ID', ukryteid, "")
		-- 	mainMenu:AddItem(thisItem)
		-- 	thisItem.CheckboxEvent = function(sender, item, status)
		-- 		if item == thisItem then
		-- 			ukryteid = not ukryteid
		-- 			TriggerServerEvent("esx_scoreboard:hidemyid", ukryteid)
		-- 		end
		-- 	end
		-- end	
		...
		if isAdmin then
			...
			if drawInfo then
				...
				if GetPlayerInvincible(ply) then
					table.insert(text,'Godmode: Tak')
				else
					table.insert(text,'Godmode: Nie')
				end
				...
				if not CanPedRagdoll(targetPed) and not IsPedInAnyVehicle(targetPed, false) and (GetPedParachuteState(targetPed) == -1 or GetPedParachuteState(targetPed) == 0) and not IsPedInParachuteFreeFall(targetPed) then
					table.insert(text,'Anty-Ragdol: Tak')
				end
			...
			if drawCoords then
				...
				SetTextOutline()
				SetTextEntry("STRING")
				AddTextComponentString("X:~s~ "..roundx)
				DrawText(0.28, 0.00)
				...
				SetTextOutline()
				SetTextEntry("STRING")
				AddTextComponentString("Y:~s~ "..roundy)
				DrawText(0.44, 0.00)
				...
				SetTextOutline()
				SetTextEntry("STRING")
				AddTextComponentString("Z:~s~ "..roundz)
				DrawText(0.60, 0.00)
				...
				SetTextOutline()
				SetTextEntry("STRING")
				AddTextComponentString("Heading:~s~ "..roundh)
				DrawText(0.40, 0.05)
				...
				SetTextOutline()
				SetTextEntry("STRING")
				AddTextComponentString("Speed: ~s~"..rounds)
				DrawText(0.40, 0.90)
				...
				SetTextOutline()
				SetTextEntry("STRING")
				AddTextComponentString("Health: ~s~"..health)
				DrawText(0.40, 0.85)
				...
				if IsPedInAnyVehicle(playerPed, 1) then
					...
					SetTextOutline()
					SetTextEntry("STRING")
					AddTextComponentString("Engine Health: ~s~"..vehenground)
					DrawText(0.0, 0.73)
					...
					SetTextOutline()
					SetTextEntry("STRING")
					AddTextComponentString("Body Health: ~s~"..vehbodround)
					DrawText(0.0, 0.69)
				end
			else
				sleep = false
