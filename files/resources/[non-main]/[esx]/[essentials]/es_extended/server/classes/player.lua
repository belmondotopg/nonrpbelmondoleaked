function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, communityservice)
	...
	self.communityservices = communityservice;
	...
	function self.setCommunityService(count)
		self.communityservices = count;
	end

	function self.getCommunityServices()
		return self.communityservices or 0
	end
	...
	function self.canCarryItem(name, count, metadata)
		...
		local canCarry = newWeight <= self.maxWeight
	...
	function self.setJob(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			exports['non']:ServerDebugPrint({type = "WARNING", message = ('Ignoring invalid .setJob() usage for "%s"'):format(self.identifier)})
		end
	end
