...
function ESX.RegisterCommand(name, group, cb, allowConsole, suggestion)
  ...
  if Core.RegisteredCommands[name] then
    exports['non']:ServerDebugPrint({type = "WARNING", message = ('Command ^5"%s" ^7already registered, overriding command'):format(name)})
...
function ESX.TriggerServerCallback(name, requestId, source, cb, ...)
  if Core.ServerCallbacks[name] then
    ...
    exports['non']:ServerDebugPrint({type = "error", message = ('Server callback ^5"%s"^0 does not exist.'):format(name)})
  end
end
...
function Core.SavePlayer(xPlayer, cb)
  ...
  MySQL.prepare(
    'UPDATE `users` SET `name` = ?, `discordid` = ?, `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `communityservices` = ? WHERE `identifier` = ?',
    {xPlayer.name, discord, json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade, xPlayer.group, json.encode(xPlayer.getCoords()),
     json.encode(xPlayer.getInventory(true)), json.encode(xPlayer.getLoadout(true)), xPlayer.getCommunityServices(), xPlayer.identifier}, function(affectedRows)
  ...
  if count > 0 then
    ...
    for i = 1, count do
      ...
      parameters[#parameters + 1] = {xPlayer.name, json.encode(xPlayer.getAccounts(true)), xPlayer.job.name, xPlayer.job.grade, xPlayer.group,
                                     json.encode(xPlayer.getCoords()), json.encode(xPlayer.getInventory(true)), json.encode(xPlayer.getLoadout(true)), xPlayer.getCommunityServices(),
                                     xPlayer.identifier}
    end
    MySQL.prepare(
      "UPDATE `users` SET `name` = ?, `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `communityservices` = ? WHERE `identifier` = ?",
      parameters, function(results)
        if results then
          if type(cb) == 'function' then
            cb()
          else
            exports['non']:ServerDebugPrint({type = "info", message = ('Saved %s %s over %s ms'):format(count, count > 1 and 'players' or 'player', (os.time() - time) / 1000000)})
...
function Core.IsPlayerAdmin(playerId)
  ...
  if xPlayer then
    if xPlayer.group == 'wlasciciel' then
      return true
    end
  end
...
local cachedDiscordData = {}
local cacheTimestamp = {}

ESX.getDiscordData = function(src)
    local discordIdentifier = GetPlayerIdentifierByType(src, "discord")
    discordIdentifier = string.gsub(discordIdentifier, "discord:", "")
    local name
    local image
    
    if cachedDiscordData[discordIdentifier] and (os.time() - cacheTimestamp[discordIdentifier]) < 600 then
        return cachedDiscordData[discordIdentifier]
    end

    PerformHttpRequest("https://discord.com/api/v9/guilds/1246158945706446889/members/"..discordIdentifier, function(err, text, headers)
        ...
        cachedDiscordData[discordIdentifier] = {name = name, image = image}
        cacheTimestamp[discordIdentifier] = os.time()
        exports['non']:ServerDebugPrint({type = "info", message = "Pobiera Profilowe od ziutka oraz nazwe."})
    end, 'GET', nil, {['Content-Type'] = 'application/json', ["Authorization"] = "Bot MTMxMDAxMTc4NjE5OTgyNjUwMw.GH8v1u.joVB4U6SmYbp4tkBlA9ioD_9oWtNhDFv2uzaYI"})
