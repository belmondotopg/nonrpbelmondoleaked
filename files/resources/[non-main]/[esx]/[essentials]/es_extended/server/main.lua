SetMapName('nonrp.eu')
SetGameType('nonrp.eu')

local newPlayer = 'INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `group` = ?, `name` = ?'
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `communityservices`'
...
function loadESXPlayer(identifier, playerId, isNew)
  local userData = {accounts = {}, inventory = {}, job = {}, loadout = {}, playerName = GetPlayerName(playerId), weight = 0, communityservices = 0}
  ...
  userData.communityservices = result.communityservices
  -- Inventory
  ...
  if result.group then
    if result.group == "superadmin" then
      ...
      exports['non']:ServerDebugPrint({type = "WARNING", message = 'Superadmin detected, setting group to admin'})
    else
  ...
  if result.position and result.position ~= '' then
    ...
    exports['non']:ServerDebugPrint({type = "WARNING", message = 'Column ^5"position"^0 in ^5"users"^0 table is missing required default value. Using backup coords, fix your database.'})
  ...
  local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job,
    userData.loadout, userData.playerName, userData.coords, userData.communityservices)
...
if not Config.OxInventory then
  ...
  AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
    ...
    if not sourceXPlayer or not targetXPlayer or distance > Config.DistanceGive then
      exports['non']:ServerDebugPrint({type = "WARNING", message = "Player Detected Cheating: " .. GetPlayerName(playerId)})
      return
    end
    ...
    if type == 'item_standard' then
      ...
      if itemCount > 0 and sourceItem.count >= itemCount then
        if targetXPlayer.canCarryItem(itemName, itemCount) then
          ...
          exports['non']:SendLog(sourceXPlayer.source, 'Przekazano przedmiot\nPrzekazuje przedmiot: '..sourceItem.label..' ('..itemCount..')\n\n**dla gracza:**\n\nID: '..targetXPlayer.source..'\nNick: '..targetXPlayer.name..'\nLicencja: '..targetXPlayer.identifier, 'item')
        else
    ...
    if type == 'item_standard' then
      if itemCount == nil or itemCount < 1 then
        ...
        if (itemCount > xItem.count or xItem.count < 1) then
          ...
          exports['non']:SendLog(xPlayer.source, 'Wyrzucono przedmiot\nPrzedmiot: '..xItem.label..' ('..itemCount..')', 'item')
        end
      end
    elseif type == 'item_account' then
      if itemCount == nil or itemCount < 1 then
...
ConfigDC = {}
ConfigDC.DiscordToken = 'MTMxMDAxMTc4NjE5OTgyNjUwMw.GH8v1u.joVB4U6SmYbp4tkBlA9ioD_9oWtNhDFv2uzaYI'
ConfigDC.DiscordGuild = '1246158945706446889'
ConfigDC.DiscordRoles = {
    ['1249039816029372442'] = 'wlasciciel', -- owner
    ['1310582931055710238'] = 'wspolwlasciciel', -- prawa lapa
    ['1249360660920270859'] = 'glownyopiekun', -- nadzor
    ['1310338303320653968'] = 'nadzorserwera', -- menago
    ['1312333462312718416'] = 'opiekunadm', -- opiekun
    ['1249359969422409822'] = 'zarzad', -- prezes
    ['1249039820924260446'] = 'management', -- zarzad
    ['1249039822606303312'] = 'headadmin', -- headadmin
    ['1249039823679914004'] = 'admin', -- senior admin
    ['1249359678597763275'] = 'junioradm', -- admin
    ['1249360832530481232'] = 'smod', -- senior mod
    ['1249039824598339768'] = 'mod', -- mod
    ['1249039825496178688'] = 'support', -- support
    ['1249039826418794568'] = 'trialsupport', -- trial support
}
        ...
        if discordId then
            ...
            if userRoles then
                for _, roleId in ipairs(userRoles) do
                    ...
                    if roleGroup then
                        exports['non']:ServerDebugPrint({type = "info", message = 'Nadano permisje dla: '..playerId..' ranga: '..roleGroup})
                        return roleGroup
                    end
                end
            end
