    ...
    local orgTeam = functions.getOrgTeam()
        ...
        CreateThread(function()
            ...
            while keepShowing do
                ESX.ShowFloatingHelpNotification('Nie zapomnij ustawić stroju organizacji przed bitką!', notifyCoords)
                Wait(1)
            end
        end)
end
...
functions.canStartBattle = function()
    ...
    for _, vehicle in ipairs(vehicles) do
        local numOfPassengers = GetVehicleNumberOfPassengers(vehicle)
        if (numOfPassengers > 0) then
            battleVehs[#battleVehs + 1] = {
                vehicle = vehicle,
                vehicleMass = GetVehicleHandlingInt(vehicle, 'CHandlingData', 'fMass')
            }
        end
        if not (IsVehicleSeatFree(vehicle, -1)) and numOfPassengers < 1 then
            battleVehs[#battleVehs + 1] = {
                vehicle = vehicle,
                vehicleMass = GetVehicleHandlingInt(vehicle, 'CHandlingData', 'fMass')
            }
...
RegisterKeyMapping('hosted', 'Menu Bitek', 'keyboard', 'F4')
