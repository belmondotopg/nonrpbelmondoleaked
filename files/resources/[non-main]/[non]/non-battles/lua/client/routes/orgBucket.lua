        ...
        while cache.player.inOrgBucket do
            if not (cache.player.inSearchingSlotBucket) and not (cache.player.inBattle) then
                if not (cache.player.customOrgBucket) then
                    ...
                    ESX.ShowHelpNotification('Aby rozpocząć hostowaną Bitkę, kliknij F4 lub wpisz komendę /hosted')
                    -- exports.HelpNotification:ShowHelpNotification('Aby rozpocząć hostowaną Bitkę, kliknij F4 lub wpisz komendę /hosted')
                end
