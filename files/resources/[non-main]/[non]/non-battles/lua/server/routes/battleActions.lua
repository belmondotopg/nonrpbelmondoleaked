...
functions.gotKicked = function(source, customCoords)
    ...
    if (playerCustomBucket) then
        TriggerClientEvent('non_radio:toggleThirdPartyAccess', source,
...
events.sendJoinInvite = function(target)
    ...
    if (xTarget.job.name == 'unemployed') then
        return xPlayer.showNotification(
                   'Ten gracz nie jest w żadnej Organizacji!')
    end
...
events.acceptInvite = function(org)
    ...
    TriggerClientEvent('non_radio:toggleThirdPartyAccess', src,
...
events.playerQuit = function(source)
    ...
    if (playerState.inOrgBucket) then
        local playerTeam = (customBucket) and cache.teams[customBucket] or
                               cache.teams[xPlayer.job.name]
...
events.leaveSlot = function(source)
    ...
    if not (pState.inOrgBucket) or not (pState.customOrgBucket) then
        return xPlayer.showNotification(
                   'Nie jesteś w żadnym buckecie organizacyjnym!')
    end
...
events.enterSearchingMode = function(source)
    ...
    elseif not (pState.customOrgBucket) then
        ...
        local playerTeam = cache.teams[pState.customOrgBucket or
                               xPlayer.job.name]
        if (playerTeam) then
...
events.acceptSearchingPlayer = function(target)
    ...
    TriggerClientEvent('non_radio:toggleThirdPartyAccess', target,
                       orgTeam.org.name, true)
...
ESX.RegisterServerCallback(eventRoute .. 'getSearchingPlayers',
                           events.getSearchingPlayers)
RegisterNetEvent(eventRoute .. 'acceptSearchingPlayer',
                 events.acceptSearchingPlayer)
