...
local initialEmbed = {
    ['avatar_url'] = 'https://i.ibb.co/YT6PYVs/1024x1024.png',
    ['title'] = 'Statystyki zabójstw podczas bitki',
    ['footer'] = {
        ['text'] = os.date() .. ' - nonRP',
        ['icon_url'] = 'https://i.ibb.co/YT6PYVs/1024x1024.png'
    }
}
...
local webhook = 'https://discord.com/api/webhooks/1319292527345401856/VoE0CncyN-53KG9XAOLngZdBDevMx6E8JZ9w2pZBHHXLyGQGsbU_7JjYKNWif-5gbHkj'

local function sendbattlewebhook(message, title)
    local embed = {
        {
            ["color"] = "9215743",
            ["title"] = "Bitka: "..title,
            ["description"] = message,
            ["footer"] = {
                ["text"] = os.date('%H:%M:%S', os.time()) .. " - Bikti Hostowane",
...
functions.incrementOrgStatsAfterBattle = function(teams)
    ...
    " WR: "..loseRatio..")", winnerTeamName.." VS "..loserTeamName)

    exports['non']:ManipulateBitkiPoints(winnerTeam, 1)
end
...
functions.insertTeamKill = function(xKiller, teamObject)
    ...
    if not (teamObject.teamKillers[xKiller.name]) then
        ...
        if (Player(xKiller.source).state.customOrgBucket) then
            ...
            teamObject.teamKillers[xKiller.name].kills = teamObject.teamKillers[xKiller.name].kills + 1
        end
    end

    if isFirstKill then
        TriggerClientEvent('InteractSound_CL:PlayOnOne', xKiller.source, 'firstblood', 0.5)
    end
end
...
events.onPlayerDeath = function(data)
    ...
    if (playerState.inBattle) then
        ...
        playersTeam.deaths = playersTeam.deaths + 1
        playersTeam.deadPlayers[#playersTeam.deadPlayers + 1] = src

        local killedByPlayer = data.killedByPlayer
        local xKiller = (killedByPlayer) and ESX.GetPlayerFromId(data.killerServerId) or {name = xPlayer.name .. ' [sam się zabił]', source = xPlayer.source}
        ...
        if (killedByPlayer) then
            if (isKillerImpostor) then
                ...
                if (playersTeam.impostors[xKiller.name].kills >= config.battleActions.maxImpostors) then
                    DropPlayer(xKiller.source, 'zabiłeś ' .. playersTeam.impostors[xKiller.name].kills .. ' swoich, to ewidentnie za dużo.')
                end
            else
        ...
        local killerOrg = (xKiller.job and xKiller.job.label) and xKiller.job.label or 'Brak organizacji'
        local victimOrg = (xPlayer.job and xPlayer.job.label) and xPlayer.job.label or 'Brak organizacji'

        for _, teamMember in ipairs(playersTeam.members) do
            TriggerClientEvent('non:showNotification', teamMember, {
                title = 'Bitki',
                text = xKiller.name.." ("..killerOrg..") zabil/a "..xPlayer.name.." ("..victimOrg..")"
            })
        end

        for _, enemyMember in ipairs(playerBattle[playerBattle.enemyTeam].members) do
               TriggerClientEvent('non:showNotification', enemyMember, {
                title = 'Bitki',
                text = xKiller.name.." ("..killerOrg..") zabil/a "..xPlayer.name.." ("..victimOrg..")"
            })
        end
