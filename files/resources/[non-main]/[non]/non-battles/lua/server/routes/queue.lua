...
events.startSearch = function(source,cb)
   ...
   -- local isBlocked = exports['island_organizations']:isBlocked(xPlayer.job.name)

    --if (isBlocked and isBlocked.isBlocked) then
     --   return functions.notifyMembers(orgTeam.members, 'Wasza organizacja ma zawias, nie możecie grać bitek! Blokada minie za: ' .. isBlocked.time .. ' minut.')
    --end
    ...
    orgTeam.battleMembers = ESX.Table.Clone(orgTeam.members)
    CreateThread(function()
        orgTeam.vehicles = {}
