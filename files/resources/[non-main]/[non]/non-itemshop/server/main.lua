	...
	if resourceName == GetCurrentResourceName() then
		CreateThread(function()
			for _, xPlayer in ipairs(ESX.GetExtendedPlayers()) do
                local result = MySQL.query.await('SELECT coins, points, inventory FROM non_coins WHERE identifier = ?', {xPlayer.identifier})
...
AddEventHandler("onResourceStop", function(ResourceName)
    if ResourceName == GetCurrentResourceName() then
        for identifier, data in pairs(PlayerData) do
            MySQL.update("UPDATE non_coins SET coins = ?, points = ?, inventory = ? WHERE identifier = ?", {data.coins, data.points, json.encode(data.inventory), identifier})
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    ...
    if result and result[1] then
        ...
        MySQL.insert.await('INSERT INTO non_coins (identifier, coins, points) VALUES (?, ?, ?) ', {xPlayer.identifier, 500, 0})
    end

    local HasNick = GetPlayerName(source) and string.find(GetPlayerName(source), 'nonrp.eu') or false
    local waitTime = HasNick and 180000 or 360000
    local notifyText = HasNick and 'Otrzymano punkty za '..ESX.Math.Round(waitTime / 60000)..' minuty gry (posiadasz nonrp.eu w nicku)' or 'Otrzymano punkty za '..ESX.Math.Round(waitTime/60000)..' minut gry (dodaj nonrp.eu do nicku steam, aby otrzymywać punkty szybciej)'
    ...
    TriggerClientEvent('non-itemshop:client:playerLoaded', _source, PlayerData[xPlayer.identifier])
end)
...
RegisterNetEvent('esx:playerDropped', function(playerId, reason)
    ...
    if data then
        MySQL.update.await("UPDATE non_coins SET coins = ?, points = ?, inventory = ? WHERE identifier = ?", {data.coins, data.points, json.encode(data.inventory), xPlayer.identifier})
        PlayerData[xPlayer.identifier] = nil
    end
end)

ESX.RegisterServerCallback('non-itemshop:callback:getSteamPhotoUrl', function(source, cb)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    cb(xPlayer.discord.image)
end)

ESX.RegisterServerCallback('non-itemshop:callback:openCase', function(source, cb, caseId)
    ...
    if caseId == "71233" then -- freecase
        local Time = MySQL.scalar.await('SELECT freeCase FROM non_coins WHERE identifier = ?', {xPlayer.identifier})
        ...
        MySQL.update.await("UPDATE non_coins SET freeCase = ? WHERE identifier = ?", {(os.time() + (24 * (60 * 60))), xPlayer.identifier})
    ...
    elseif caseId == "71234" then -- booster case
        if not exports['non']:IsRolePresent(source, Config.BoosterRoleID) then
            cb({
                success = false,
        ...
        local Time = MySQL.scalar.await('SELECT boosterCase FROM non_coins WHERE identifier = ?', {xPlayer.identifier})
        if Time and Time > os.time() then
            cb({
                success = false,
        ...
        MySQL.update.await("UPDATE non_coins SET boosterCase = ? WHERE identifier = ?", {(os.time() + (24 * (60 * 60))), xPlayer.identifier})
...
ESX.RegisterServerCallback("non-itemshop:callback:withdrawItem", function(source, cb, data)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if ItemData.category == "car" then
        ...
        local model = ItemData.name
            ...
            exports['non']:SendLog(xPlayer.source, "Ziomek odebral autko model:"..model.." plate: "..plate, 'sklepcoins')


    elseif ItemData.category == "item" then
        exports['non']:SendLog(xPlayer.source, "Ziomek odebral item resp:"..ItemData.name.." ilosc: "..ItemData.count, 'sklepcoins')
    ...
    TriggerClientEvent('non-itemshop:client:updatePlayerData', source, PlayerData[xPlayer.identifier])
end)

RegisterNetEvent("non-itemshop:server:keepItem", function(item)
    ...
    if item.category == "voucher" then
        ...
        MySQL.update.await("UPDATE non_coins SET inventory = ? WHERE identifier = ?", {json.encode(PlayerData[xPlayer.identifier].inventory), xPlayer.identifier})
    end

    TriggerClientEvent("non-itemshop:client:updateLiveDrop", -1, {
        title = item.title,
    ...
    TriggerClientEvent('non-itemshop:client:updatePlayerData', xPlayer.source, PlayerData[xPlayer.identifier])
end)

ESX.RegisterServerCallback('non-itemshop:callback:sellItem', function(source, cb, item)
    local SellType = nil
    ...
    exports['non']:SendLog(xPlayer.source, "Ziomek sprzedal item: "..item.name.." za: "..item.price.." nowe coinsy: "..NewCoins, 'sklepcoins')
...
--     TriggerClientEvent('non-itemshop:client:updatePlayerData', TargetId, PlayerData[xPlayer.identifier])
...
ESX.RegisterServerCallback('non-itemshop:callback:getFreeCaseTime', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local Time = MySQL.scalar.await('SELECT freeCase FROM non_coins WHERE identifier = ?', {xPlayer.identifier})
...
ESX.RegisterServerCallback('non-itemshop:callback:buyItem', function(source, cb, data)
    local xPlayer = ESX.GetPlayerFromId(source)
    local ItemName = data["item"].name
    local BoughtCount = MySQL.scalar.await('SELECT count FROM non_coins_shop WHERE name = ?', {ItemName})
...
ESX.RegisterServerCallback('non-itemshop:callback:discount', function(source, cb, data)
    local xPlayer = ESX.GetPlayerFromId(source)
    local UsedCode = string.upper(data.code)
    
    local RewardFromCode = MySQL.scalar.await('SELECT reward FROM non_coins_promocodes WHERE name = ?', {UsedCode})
    ...
    local UsedCodes = MySQL.scalar.await('SELECT usedCodes FROM non_coins WHERE identifier = ?', {xPlayer.identifier})
    ...
    MySQL.update.await("INSERT INTO non_coins_promocodes (name, usages) VALUES (:name, :usages) ON DUPLICATE KEY UPDATE usages = usages + :usages;", {
        name = UsedCode,
        usages = 1
    })
    UsedCodes[#UsedCodes + 1] = UsedCode
    MySQL.update.await("UPDATE non_coins SET usedCodes = ? WHERE identifier = ?", {json.encode(UsedCodes), xPlayer.identifier})
...
RegisterNetEvent('non-itemshop:UpdatePlayer', function()
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    if not xPlayer then return end
    TriggerClientEvent('non-itemshop:client:updatePlayerData', _source, PlayerData[xPlayer.identifier])
end)
...
AddItemCount = function(name, count)
    MySQL.update.await("INSERT INTO non_coins_shop (name, count) VALUES (:name, :count) ON DUPLICATE KEY UPDATE count = count + :count;", {
        name = name, 
        count = count
    })
end
...
CreateThread(function()
    for k, v in pairs(Config.Cases) do
        ...
        print('Łączne szasne na przedmioty w skrzyni '..v.title..': '..total)
    end
end)
...
GeneratePlate = function()
    ...
    if existingPlates[generatedPlate] then
        ...
        local result = MySQL.Async.fetchScalar('SELECT identifier FROM owned_vehicles WHERE plate LIKE @plate', {
