...
function carKeys(playerPed)
    ...
    if vehicle and DoesEntityExist(vehicle) then
        ...
        ESX.TriggerServerCallback('non-locksystem:checkKeys', function(hasKey, canSearch)
            if hasKey then
            ...
            elseif canSearch then
                if Ped.InVehicle then
                    ...
                    if chance <= 70 then
                        TriggerServerEvent('non-locksystem:addCarKeys', plate)
                        ESX.ShowNotification('Znaleziono klucze do tego pojazdu', 'info')
                    else
                        TriggerServerEvent('non-locksystem:blockCarSearch', plate)
...
function longRangeKeys(playerPed)
    ESX.TriggerServerCallback('non-locksystem:getAllKeys', function(carKeysTable)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'locksystem_keys', {
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'locksystem_options', {
						...
						elseif data2.current.value == 'dropKeys' then
							menu.close()
							menu2.close()
							TriggerServerEvent('non-locksystem:removeCarKeys', data.current.value)
...
function giveTempKeys(playerPed, plate)
	...
	if #playersInArea >= 1 then
		...
		function(data, menu)
			...
			TriggerServerEvent('non-locksystem:addCarKeys', plate, data.current.value)
		end, function(data, menu)
			menu.close()
		end)
	else
