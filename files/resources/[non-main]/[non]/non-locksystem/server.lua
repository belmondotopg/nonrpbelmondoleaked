...
ESX.RegisterServerCallback('non-locksystem:checkKeys', function(source, cb, plate)
...
ESX.RegisterServerCallback('non-locksystem:getAllKeys', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then return end
	cb(globalKeysTable[xPlayer.identifier])
end)

RegisterServerEvent("non-locksystem:blockCarSearch")
AddEventHandler("non-locksystem:blockCarSearch", function(plate)
    local plate = string.lower(plate)
    if not searchedCars[plate] then
        searchedCars[plate] = true
    end
...
RegisterServerEvent("non-locksystem:addCarKeys")
AddEventHandler("non-locksystem:addCarKeys", function(plate, target)
    local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("non-locksystem:removeCarKeys")
AddEventHandler("non-locksystem:removeCarKeys", function(plate)
    local xPlayer = ESX.GetPlayerFromId(source)
    local plate = string.lower(plate)
    if globalKeysTable[xPlayer.identifier] then
        for k, v in pairs(globalKeysTable[xPlayer.identifier]) do
