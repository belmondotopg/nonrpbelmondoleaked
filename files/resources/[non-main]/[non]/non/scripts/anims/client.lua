...
function startAnim(libr, anim, loop, propTable)
	...
	if exports['non']:isPedAble(ped) then
...
RegisterNUICallback("setBinding", function(data)
	if data.binding then
		for key, anim in pairs(data.binding) do
			local remove = NON.RegisterButton(key, function()
				playAnim(anim)
			end)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'play_synced',
	{
		...
		if allowed then
			if timer < GetGameTimer() then
				...
				if player then
					TriggerServerEvent('non-anims:requestSynced', player, data2.current.id)
...
RegisterNetEvent('non-anims:syncRequest', function(requester, id)
	...
	CreateThread(function()
		...
		local menu = ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'synced_animation_request', {
			...
			if data.current.value then
				resetmenu = true
				TriggerServerEvent('non-anims:syncAccepted', requester, id)
			else
				resetmenu = true
				TriggerServerEvent('non-anims:cancelSync', requester)
...
RegisterNetEvent('non-anims:playSynced', function(serverid, id, type)
