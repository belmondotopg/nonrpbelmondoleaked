...
local function OpenShopMenuBarber()
	hasPaid = false
	...
	TriggerEvent('esx_skin:openRestrictedMenu', function(data, menu)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data.current.value == 'yes' then
				ESX.TriggerServerCallback('non-barbershop:pay', function(paid)
					if paid then
...
for k, v in pairs(Config['barbershop'].Shops) do
	NON.RegisterPlace(v, {size = vector3(2.0, 2.0, 0.3)}, "otworzyć menu fryzjera",
	function()
		OpenShopMenuBarber()
	end,
	function()
