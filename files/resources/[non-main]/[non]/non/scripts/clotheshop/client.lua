	...
	TriggerEvent('esx_skin:openRestrictedMenu', function(data, menu)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data.current.value == 'yes' then
				ESX.TriggerServerCallback('non-clotheshop:buyClothes', function(bought)
					if bought then
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing', {
							...
							if data2.current.value == 'yes' then
								ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
									...
									TriggerEvent('skinchanger:getSkin', function(skin)
										TriggerServerEvent('non-dressings:saveOutfit', data3.value, skin)
...
function PlayerDressings()
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dress_menu',
	{
		...
		align    = 'center',
		elements = {
            {label = 'Zapisane Ubrania', value = 'player_dressing'},
...
for k, v in pairs(Config['clotheshop'].Shops) do
	NON.RegisterPlace(v, {size = vector3(2.0, 2.0, 0.3)}, "otworzyć sklep z ubraniami",
	function()
		OpenSelectMenu()
	end,
	function()
