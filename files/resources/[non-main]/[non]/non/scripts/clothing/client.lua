...
function OpenClothingMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'non_clothing_menu', {
		title = 'Ubrania',
		align = 'right',
		elements = {
...
function ClothingMainFunction(accessory, skin)
    ...
    if accessory == 'torso' then
        if skin['torso_1'] and (skin['torso_1'] ~= defaultVariants[skin.sex]['torso'] or skin['tshirt_1'] ~= defaultVariants[skin.sex]['tshirt']) then
            ...
            TriggerServerEvent('non:setDataClothing', accessory, {
            ...
            ESX.TriggerServerCallback('non:getDataClothing', function(save)
                if save then
                    TriggerEvent('skinchanger:loadClothes', skin, {
...
ESX.RegisterInput('clothing', 'Zdejmowanie/zakładanie ubrań', 'keyboard', 'K', function()
