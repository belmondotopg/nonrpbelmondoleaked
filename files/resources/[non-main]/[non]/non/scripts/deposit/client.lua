...
for i, v in ipairs(Config['deposit'].Zones) do
	NON.RegisterPlace(v, {size = vector3(2.0, 2.0, 0.3)}, "otworzyć schowek",
	function()
...
RegisterNetEvent("PaT:JEBACCIE", function()
	exports['non-hud']:ProgressBar(5000, 'Otwierasz schowek..')
	depositOptions()
end)
...
function depositOptions()
    local elements = {
        ...
        {label = 'Zapisane ubrania', value = 'clothes'},
		{label = 'Zmień wygląd postaci', value = 'buy_clothes'},
    }
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'deposit', {
		...
		elseif data.current.value == 'buy_clothes' then
			-- OpenShopMenuClothes()
			exports['non']:OpenShopMenuClothes()
        end
...
function OpenDepositInventoryMenu()
	ESX.UI.Menu.CloseAll()
	ESX.TriggerServerCallback('non_deposits:getDepositInventory', function(inventory)
            ...
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'deposit_inventory', {
                ...
                ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_get_item_count', {
                    ...
                    if not quantity then
                        ...
                        TriggerServerEvent('non_deposits:getItem', data.current.type, data.current.value, quantity)
                        menu2.close()
...
function OpenPlayerInventoryMenu()
	ESX.TriggerServerCallback('non_deposits:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'deposit_player_inventory', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_put_item_count', {
				...
				if not quantity then
                    ...
                    TriggerServerEvent('non_deposits:putItem', data.current.type, data.current.value, quantity)
...
function PlayerDressings()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dress_menu',
	{
		...
		if data.current.value == 'player_dressing' then 
			ESX.TriggerServerCallback('non-dressings:getPlayerDressing', function(dressing)
				local elements = {}
				...
				if #elements > 0 then
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
						...
						TriggerEvent('skinchanger:getSkin', function(skin)
							ESX.TriggerServerCallback('non-dressings:getPlayerOutfit', function(clothes)
		...
		elseif data.current.value == 'remove_cloth' then
			ESX.TriggerServerCallback('non-dressings:getPlayerDressing', function(dressing)
				...
				if #elements > 0 then
					...
					function(data2, menu2)
						menu2.close()
						TriggerServerEvent('non-dressings:removeOutfit', data2.current.value)
