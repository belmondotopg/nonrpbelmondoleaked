...
RegisterNetEvent('non:onEnergyDrink', function()
...
RegisterNetEvent('non:onSmokeWeed', function()
...
RegisterNetEvent('non:onMethPooch', function()
...
RegisterNetEvent('non:onRecoilDrug', function()
	local needThread = false
...
RegisterNetEvent('non:onRepairKit', function(triggered)
	local playerPed = PlayerPedId()
	...
	if DoesEntityExist(vehicle) then
		if not triggered then
			TriggerServerEvent('non:repairKitAction', 'remove')
		end
		...
		showProgress({title = 'Naprawianie pojazdu', time = 20000}, function(isDone)
			if isDone then
				...
				if not triggered then
					TriggerServerEvent('non:repairKitAction', 'give')
				end
			end
			inRepair = false
...
RegisterNetEvent('non:useVest', function(value)
