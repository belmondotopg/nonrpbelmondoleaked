...
AddEventHandler('non:getLicenses', function(identifier, cb)
    cb(getLicenses(identifier))
end)

AddEventHandler('non:checkLicense', function(identifier, license, cb)
    cb(checkLicense(identifier, license))
end)

AddEventHandler('non:addLicense', function(identifier, license, temporary, cb)
    cb(addLicense(identifier, license, temporary))
end)

AddEventHandler('non:removeLicense', function(identifier, license, cb)
    cb(removeLicense(identifier, license))
end)

ESX.RegisterCommand('addlicense', 'headadmin', function(xPlayer, args, showError)
...
ESX.RegisterCommand('removelicense', 'headadmin', function(xPlayer, args, showError)
	if args.playerId and args.license then
...
ESX.RegisterServerCallback('non:checkLicense', function(source, cb, target, licenseType)
    local xPlayer = nil
    if not target then
...
ESX.RegisterServerCallback('non:getlicenses', function(source, cb)
