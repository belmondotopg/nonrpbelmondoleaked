...
RegisterNetEvent('esx:playerLoaded', function(player, xPlayer, isNew)
	if isNew then
		exports['non-garages']:addPlayerVehicle(xPlayer.identifier, "josgtr34", exports['non-itemshop']:GeneratePlate())

		for k,v in pairs(itemy) do
			xPlayer.addInventoryItem(k, v)
		end

		Wait(1500)

		exports['non-itemshop']:addPlayerCoins(xPlayer.source, 500)
	end
end)

RegisterNetEvent('non:jestemkurwom:nadaj:token')
AddEventHandler('non:jestemkurwom:nadaj:token', function(bucket)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	SetPlayerRoutingBucket(xPlayer.source, math.random(0, bucket))
end)
...
elseif ESX.GetConfig().Multichar == true then
	...
	AddEventHandler('esx_identity:completedRegistration', function(source)
		TriggerEvent('esx:onPlayerJoined', source, PREFIX)
		ESX.Players[GetIdentifier(source)] = true
	end)
