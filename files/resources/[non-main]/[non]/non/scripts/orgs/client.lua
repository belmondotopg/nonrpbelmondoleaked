local perms = {
	...
	{label = "Wyciąganie zestawów",   name = "withdraw_kit",    value = false},
	...
	{label = "Zarządzanie zestawami", name = "kits_menager",    value = false},
...
CreateThread(function()
	...
	for k, v in pairs(Config["orgs"].Zones) do
		NON.RegisterPlace(v, {size = vector3(5.0, 5.0, 0.3)}, "otworzyć menu organizacji",
		function()
...
function OpenOrgMenu()
	...
	if ESX.PlayerData.job.grade == 1 or ESX.PlayerData.job.grade_permissions["withdraw_item"] or ESX.PlayerData.job.grade_permissions["deposit_item"] or ESX.PlayerData.job.grade_permissions["withdraw_kit"] or ESX.PlayerData.job.grade_permissions["kits_menager"] then
...
function OpenBossMenu()
	...
	if ESX.PlayerData.job.grade == 1 then
		...
		elements[#elements+1] = {label = '<span style="color:red;">Zmień nazwę organizacji</span>', value = "rename"}
	...
	ESX.UI.Menu.Open("default", GetCurrentResourceName(), "OrgBossMenu", {
		...
		elseif data.current.value == "remove" then
			...
			ESX.UI.Menu.Open("default", GetCurrentResourceName(), "Orgremove", {
				...
				if data2.current.value then
					ESX.TriggerServerCallback("orgs:RemoveOrg")
				else
					OpenBossMenu()
				end
			end, function(data2, menu2)
		...
		elseif data.current.value == "rename" then
			...
			ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "renameMenu", {
				title = "Nowa nazwa twojej organizacji"
			}, function(data2, menu2)
				if data2.value then
					if string.len(data2.value) > 15 then
						TriggerEvent('non:showNotification', {
					...
					elseif string.len(data2.value) < 4 then
						...
						ESX.TriggerServerCallback("orgs:RenameOrg", function(data)
							if data then
								TriggerEvent('non:showNotification', {
									type = 'success',
									title = 'Organizacje',
									text = 'Pomyślnie zmieniono nazwę organizacji z '..data.oldname..' na '..data.newname
								})
								menu2.close()
							else
								TriggerEvent('non:showNotification', {
		...
		elseif data.current.value == 'save_wear' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'save_wear_name', {
				...
				TriggerEvent('skinchanger:getSkin', function(skin)
					TriggerServerEvent('non-dressings:saveSharedOutfit', data2.value, skin, ESX.PlayerData.job.name)
					TriggerEvent('non:showNotification', {
						type = 'success',
						title = 'Organizacje',
						...
						if #secondClothesElements > 0 then
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'saved_wear_menu_choose', {
								...
								elseif data3.current.value == 'delete_wear' then
									TriggerServerEvent('non-dressings:removeSharedOutfit', data2.current.value, ESX.PlayerData.job.name)
									TriggerEvent('non:showNotification', {
										type = 'success',
										title = 'Organizacje',
	...
	ESX.UI.Menu.Open("default", GetCurrentResourceName(), "OrgSpecifiedMemberMenu", {
		...
		elseif data.current.value == "kickMember" then
			...
			ESX.UI.Menu.Open("default", GetCurrentResourceName(), "Orgtruefalse3Menu", {
				...
				menu2.close()
				if data2.current.value then
					ESX.TriggerServerCallback("orgs:KickMember", xPlayer)
					TriggerEvent('non:showNotification', {
						type = 'success',
						title = 'Organizacje',
...
function OpenStashBoard()
	...
	if ESX.PlayerData.job.grade == 1 or ESX.PlayerData.job.grade_permissions["withdraw_kit"] or ESX.PlayerData.job.grade_permissions["kits_menager"] then
	...
	ESX.UI.Menu.Open("default", GetCurrentResourceName(), 'OrgStashBoardMenu', {
		...
		elseif data.current.value == 'put_all_in_stock' then
			menu.close()
			ESX.TriggerServerCallback('orgs:putAllInStock')
			OpenOrgMenu()
...
ESX.RegisterInput("orgs:quickactions", "Menu organizacji", "keyboard", "F6", function()
	ESX.TriggerServerCallback("orgs:IsSuspended", function(time)
		...
		local bitkaPerms = ESX.PlayerData.job.grade == 1 or ESX.PlayerData.job.grade_permissions["bitki_menager"]
		...
		ESX.UI.Menu.Open("default", GetCurrentResourceName(), 'org_f6', {
			...
			elseif data.current.value == 'bitki' then
				if data.current.bool then
					OpenBitkiMenu()
				end
			else
				TriggerEvent('non:showNotification', {
