local price = 1250000
local renamePrice = 500000
local MaxKitCount = 10

local globalWebHookLink = ''
...
ESX.RegisterServerCallback("orgs:RenameOrg", function(src, cb, JobLabel)
    local xPlayer = ESX.GetPlayerFromId(src)
    local cash, bank = xPlayer.getAccount("money"), xPlayer.getAccount("bank")
    local useBank, useMoney = false, false

    if bank.money >= renamePrice then
        useBank = true
    elseif cash.money >= renamePrice then
        useMoney = true
    end

    local oldJobLabel = Labeles[xPlayer.job.name]
    local foundLabel = false

    for jobName, label in pairs(Labeles) do
        if string.lower(JobLabel) == string.lower(label) then
            foundLabel = true
            break
        end
    end
    ...
    if foundLabel then
        TriggerClientEvent('non:showNotification', src, {
            type = 'error',
            title = 'Organizacje',
            text = "Ta nazwa org jest już zajęta"
        })
        return
    end
    ...
    if not useBank and not useMoney then
        ...
        if useBank then
            xPlayer.removeAccountMoney("bank", renamePrice)
        else
            xPlayer.removeAccountMoney("money", renamePrice)
        end

        Labeles[xPlayer.job.name] = JobLabel
        MySQL.update.await("UPDATE jobs SET label = ? WHERE name = ?", {JobLabel, xPlayer.job.name})
        ESX.RefreshJobs()

        local xPlayers = ESX.GetExtendedPlayers("job", xPlayer.job.name)
        for i = 1, #xPlayers do
            xPlayers[i].setJob(xPlayer.job.name, xPlayers[i].job.grade)
        end

        cb({newname = JobLabel, oldname = oldJobLabel})
    end
end)
...
ESX.RegisterServerCallback("orgs:KickMember", function(src, tPlayer)
    local newtPlayer = ESX.GetPlayerFromIdentifier(tPlayer.identifier)
    if newtPlayer then
        tPlayer = newtPlayer
...
ESX.RegisterServerCallback("orgs:RemoveOrg", function(src)
    local xPlayer = ESX.GetPlayerFromId(src)
...
ESX.RegisterServerCallback('orgs:putAllInStock', function(src)
	local xPlayer = ESX.GetPlayerFromId(src)
    ...
    -- SendLogToDiscord(globalWebHookLink, 'Schowano wszystkie przedmioty - '..xPlayer.job.name..' ['..xPlayer.job.label..']', '**ID: **'..xPlayer.source..'\n**Nick: **'..xPlayer.name..'\n**Licencja: **'..xPlayer.identifier..'\n**Przedmioty: **'..itemsList, 5763719)
end)
...
ESX.RegisterServerCallback('orgs:putStockItems', function(src, cb, itemName, count)
    ...
    if sourceItem.count >= count and count > 0 then
        ...
        -- SendLogToDiscord(globalWebHookLink, 'Schowano przedmiot - '..xPlayer.job.name..' ['..xPlayer.job.label..']', '**ID: **'..xPlayer.source..'\n**Nick: **'..xPlayer.name..'\n**Licencja: **'..xPlayer.identifier..'\n**Przedmiot: **'..sourceItem.label..' ('..count..')', 5763719)
    else
        ...
        if Stashes[xPlayer.job.name][i] and Stashes[xPlayer.job.name][i].name == itemName then
            if count > 0 and Stashes[xPlayer.job.name][i].count >= count then
                ...
                if xPlayer.canCarryItem(itemName, count) then
                    ...
                    -- SendLogToDiscord(globalWebHookLink, 'Wyjęto przedmiot - '..xPlayer.job.name..' ['..xPlayer.job.label..']', '**ID: **'..xPlayer.source..'\n**Nick: **'..xPlayer.name..'\n**Licencja: **'..xPlayer.identifier..'\n**Przedmiot: **'..xPlayer.getInventoryItem(itemName).label..' ('..count..')', 15548997)
                else
...
ESX.RegisterServerCallback('orgs:putMoney', function(src, cb, count)
    ...
    if count <= xPlayer.getMoney() then
        ...
        -- SendLogToDiscord(globalWebHookLink, 'Schowano pieniądze - '..xPlayer.job.name..' ['..xPlayer.job.label..']', '**ID: **'..xPlayer.source..'\n**Nick: **'..xPlayer.name..'\n**Licencja: **'..xPlayer.identifier..'\n**Ilość: **'..count..'$', 5763719)
    end
...
ESX.RegisterServerCallback('orgs:getMoney', function(src, cb, count)
    ...
    if count <= Accounts[xPlayer.job.name] then
        ...
        -- SendLogToDiscord(globalWebHookLink, 'Wyjęto pieniądze - '..xPlayer.job.name..' ['..xPlayer.job.label..']', '**ID: **'..xPlayer.source..'\n**Nick: **'..xPlayer.name..'\n**Licencja: **'..xPlayer.identifier..'\n**Ilość: **'..count..'$', 15548997)
    end
...
function ManipulateBitkiPoints(job, points)
    local preLvl = 0
    for lvl, points2 in ipairs(Lvls) do
        if BitkiPoints[job] >= points2 then
            preLvl = lvl
        end
    end
    ...
    local futureLvl = 0
    for lvl, points2 in ipairs(Lvls) do
        if BitkiPoints[job] + points >= points2 then
            futureLvl = lvl
        end
    end
    ...
    if futureLvl > preLvl then
        for i = 1, #orgPlayers do
            TriggerClientEvent('non:showNotification', orgPlayers[i].source, {
                type = 'info',
                duration = 10000,
                title = 'Bitki',
                text = 'Twoja organizacja awansowała ['..preLvl..' -> '..futureLvl..'] LVL ('..(points > 0 and ('+'..points) or points)..') PKT'
            })
        end
    elseif futureLvl < preLvl then
        for i = 1, #orgPlayers do
            TriggerClientEvent('non:showNotification', orgPlayers[i].source, {
                type = 'info',
                duration = 10000,
                title = 'Bitki',
                text = 'Twoja organizacja degradowała ['..preLvl..' -> '..futureLvl..'] LVL ('..(points > 0 and ('+'..points) or points)..') PKT'
            })
        end
    elseif points > 0 then
        for i = 1, #orgPlayers do
            TriggerClientEvent('non:showNotification', orgPlayers[i].source, {
                ...
                duration = 10000,
                title = 'Bitki',
                text = 'Twoja organizacja zyskała ('..(points > 0 and ('+'..points) or points)..') PKT ['..futureLvl..'] LVL'
            })
        end
    elseif points < 0 then
        for i = 1, #orgPlayers do
            TriggerClientEvent('non:showNotification', orgPlayers[i].source, {
                ...
                duration = 10000,
                title = 'Bitki',
                text = 'Twoja organizacja straciła ('..(points > 0 and ('+'..points) or points)..') PKT ['..futureLvl..'] LVL'
            })
        end
    end
    
    BitkiPoints[job] += points
...
exports('ManipulateBitkiPoints', ManipulateBitkiPoints)
...
ESX.RegisterCommand('zawiesorg', 'opiekunadm', function(xPlayer, args, showError)
    ...
    elseif ESX.GetJobs()[args.job] then
        ...
        exports['non']:SendLog(xPlayer.source, string.format('Użyto komendy /zawiesorg %s %s', args.job, args.time), 'org')

        xPlayer.showNotification("Wyciszono org: "..args.job)
        --TriggerClientEvent('non:chatMessage', xPlayer.source, 'rgb(255, 0, 0)', 'fa-solid fa-volume-xmark', 'WYCISZENIE', xPlayer.name..' wyciszył cię do '..os.date('%Y-%m-%d %H:%M:%S', expireTime))
    else
...
ESX.RegisterServerCallback('orgs:EquipKit', function(src, cb, iteration, tsrc)
    ...
    if Kits[xPlayer.job.name][iteration] then
        ...
        for i = 1, #kitItems do
            ...
            if foundIteration and stashItems[foundIteration].count >= kitItems[i].count then
                if tPlayer.canCarryItem(kitItems[i].item, kitItems[i].count) then
                    ...
                    itemsList = itemsList..Stashes[xPlayer.job.name][foundIteration].label.." x"..kitItems[i].count.."\n"
        ...
        -- SendLogToDiscord(globalWebHookLink, 'Wyjęto kit - '..xPlayer.job.name..' ['..xPlayer.job.label..']', '**ID: **'..xPlayer.source..'\n**Nick: **'..xPlayer.name..'\n**Licencja: **'..xPlayer.identifier..'\n**Kit: **'..Kits[xPlayer.job.name][iteration].label..'\n**Przedmioty: **'..itemsList, 9807270)
    end
...
MySQL.ready(function()
    CreateThread(function()
        ...
        for top = 1, 10 do
            ...
            newPlayerTop[#newPlayerTop+1] = {label = tempBestPlayer, points = tempBestPlayerPoints}
        end
        finalPlayersTop = newPlayerTop
    end)
...
ESX.RegisterCommand('top', 'user', function(xPlayer, args, showError)
    TriggerClientEvent("non:OpenTopka", xPlayer.source, BitkiPoints, CapturedStrefa, Labeles, finalPlayersTop, {label = GlobalPointsTable[xPlayer.identifier].name, points = GlobalPointsTable[xPlayer.identifier].points}, BitkiPoints[xPlayer.job.name], Labeles[xPlayer.job.name])
