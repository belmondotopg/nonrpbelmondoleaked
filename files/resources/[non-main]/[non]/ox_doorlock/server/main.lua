...
local function setDoorState(id, state, lockpick)
	...
	if door then
		...
		if source then
			...
			TriggerClientEvent('non:showNotification', source, {
				type = "info",
				duration = 5000,
				text = state == 0 and 'Nie mozesz otworzyc tych drzwi' or 'Nie mozesz zamknac tych drzwi'
			})
...
ESX.RegisterCommand('doorlock', 'wlasciciel', function(xPlayer, args, showError)
	if xPlayer.group == "wlasciciel" then
		TriggerClientEvent('ox_doorlock:triggeredCommand', xPlayer.source)
	end
end)