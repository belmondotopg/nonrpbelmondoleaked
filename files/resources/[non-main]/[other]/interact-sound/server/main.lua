...
AddEventHandler('InteractSound_SV:PlayOnOne', function(clientNetId, soundFile, soundVolume)
  local src = source
  exports['non']:ServerDebugPrint({type = "WARNING", message = ('%s attempted to trigger InteractSound_SV:PlayOnOne'):format(GetPlayerName(src))})
end)
...
AddEventHandler('InteractSound_SV:PlayOnAll', function(soundFile, soundVolume)
  local src = source
  exports['non']:ServerDebugPrint({type = "WARNING", message = ('%s attempted to trigger InteractSound_SV:PlayOnAll'):format(GetPlayerName(src))})
end)
...
AddEventHandler('InteractSound_SV:PlayWithinDistance', function(maxDistance, soundFile, soundVolume)
  ...
  if maxDistance < DistanceLimit then
    ...
    exports['non']:ServerDebugPrint({type = "WARNING", message = ('%s attempted to trigger InteractSound_SV:PlayWithinDistance over the distance limit ' .. DistanceLimit):format(GetPlayerName(src))})
  end
end)
